...
| graphql-ruby       | 2.5.22            | GraphQL Query / Mutation / Subscription         |
| PostgreSQL         | 17                | メインデータストア（pg_trgm で部分一致検索を高速化） |
| Redis              | 7                 | JWT Refresh Token 管理 / Action Cable Pub/Sub   |
| jwt_sessions       | 3.2.4             | Access Token + Refresh Token 認証               |
| Active Storage     | Rails 標準        | アイコン画像のローカル保存（S3 切替可）         |
| RSpec + FactoryBot | 7.1.1 / 6.5.1     | テスト                                          |
| rack-cors          | 3.0.0             | CORS 設定（フロントエンド別オリジン対応）       |
| rack-attack        | 6.8.0             | レート制限（認証: 5 req/min、投稿: 30 req/min など） |
...
| `/users/:username`             | プロフィール（投稿・いいねのタブ切替）                     | 必須                               |
| `/users/:username/connections` | フォロー / フォロワー一覧（`?tab=following` でフォロー中） | 必須                               |
...
この Rails アプリは `dotenv-rails` を使っていないため、`.env` は自動では読み込まれません。デフォルト値のまま開発する場合、この手順は確認用です。値を上書きしたい場合は、Rails サーバー起動前に `set -a; source .env; set +a` などでシェルに読み込んでください。
...
> **補足:** 現状の `backend/bin/dev` は Rails サーバーのみを起動します。フロントエンドの Vite 開発サーバーは別ターミナルで `pnpm dev` を実行してください。
